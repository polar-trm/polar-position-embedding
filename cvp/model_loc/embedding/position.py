...
# CVP版本 为了修改简单所以没有改类的名字
class PolarPostionEmbedding(nn.Module):
    def __init__(self, voc_num, d_model):
        super(PolarPostionEmbedding, self).__init__()
        self.frequency_emb = torch.nn.Embedding(voc_num, d_model)
        self.initial_phase_emb = torch.nn.Embedding(voc_num, d_model)

    
    def forward(self, amplitude, x):
        frequency = self.frequency_emb(x)
        sent_len = x.size(-1)
        pos_seq = torch.arange(1, sent_len + 1, 1.0, device=x.device)
        pos_seq = pos_seq.unsqueeze(0).unsqueeze(-1)
        pos_seq = pos_seq.repeat([x.size(0), 1, amplitude.size(-1)])
        dimension_bais = self.initial_phase_emb(x) % (2*math.pi)
